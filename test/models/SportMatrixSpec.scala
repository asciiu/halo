...
class SportMatrixSpec extends Specification {
  ...
  "A SportBookMatrix class" should {
    ...
    "keep tabs on all booknames that are entered" in {
      ...
      bookNames must beEqualTo(Seq("Nitrogen Sports"))
    }
    "contain the correct match names" in {
      val keys = List(
        ...
        "Utah Jazz vs Golden State Warriors: Over 209 vs Under 209",
        "Utah Jazz vs Golden State Warriors: Over 209.5 vs Under 209.5")
    ...
    "contain the correct odds" in {
      ...
      allOdds must beEqualTo(List(SportsBookOdds(bookname1,2.096,1.786)))
    }
    "return highest number for option a" in {
      ...
      matchOdds must beEqualTo(Some(bookname1, 3.886))
    }
    "return highest number for option b" in {
      val key = "Denver Nuggets vs Los Angeles Clippers: Denver Nuggets ML vs Los Angeles Clippers ML"
      val matchOdds = matrix.highB(key)
      matchOdds must beEqualTo(Some(bookname1, 1.283))
    }
  }
}
